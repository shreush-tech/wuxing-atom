...
export function CosmicEnergySystem(){
  ...
  const curves=useMemo(()=>[new Orbit(3.5,2.7,new THREE.Euler(1.05,.08,.02)),new Orbit(3.8,3.15,new THREE.Euler(.22,.36,.02)),new Orbit(3.9,3.0,new THREE.Euler(.72,-.18,.64)),new Orbit(3.0,2.42,new THREE.Euler(.44,.56,-.46)),new Orbit(3.35,2.58,new THREE.Euler(.92,-.42,-.34))],[])
  const goldGeo=useMemo(()=>curves.map((c,i)=>new THREE.TubeGeometry(c,72,i===1?.060:.050,7,true)),[curves])
  const coreGeo=useMemo(()=>curves.map(c=>new THREE.TubeGeometry(c,72,.014,5,true)),[curves])
  ...
  useFrame(({clock})=>{
    ...
    for(let i=0;i<18;i++){const c=curves[i%curves.length],u=(t*(.040+(i%4)*.006)+i*.071)%1,p=c.getPoint(u);dummy.position.copy(p);dummy.scale.setScalar(i%5===0?.115:.078);dummy.updateMatrix();movers.current?.setMatrixAt(i,dummy.matrix);dummy.scale.setScalar(i%5===0?.055:.035);dummy.updateMatrix();cores.current?.setMatrixAt(i,dummy.matrix)}
  ...
  return <group ref={glow}>
    ...
    <instancedMesh ref={movers} args={[undefined,undefined,18]}><sphereGeometry args={[1,8,6]}/><meshBasicMaterial color="#ffd964" transparent opacity={.74} depthWrite={false} blending={THREE.AdditiveBlending}/></instancedMesh>
    <instancedMesh ref={cores} args={[undefined,undefined,18]}><sphereGeometry args={[1,7,5]}/><meshBasicMaterial color="#ffffff" transparent opacity={.95} depthWrite={false} blending={THREE.AdditiveBlending}/></instancedMesh>
  </group>
}
