...
export function FinalResultExperience(){
  ...
  const back=()=>{document.querySelector('.app')?.scrollIntoView({behavior:'smooth',block:'start'});window.history.replaceState(null,'',location.pathname+location.search)}
  const openKnowledge=()=>window.dispatchEvent(new Event('reush-open-knowledge'))
  const goTreatment=()=>document.querySelector('.result-practical-layer')?.scrollIntoView({behavior:'smooth',block:'center'})
  return <section id="resultado" className="final-result parchment-result">
    <div className="result-top-actions"><button type="button" className="result-back" onClick={back}>← Voltar à constelação</button><button type="button" className="result-close" onClick={back} aria-label="Fechar resultado">×</button></div>
    ...
    <div className="result-pathways"><button type="button" onClick={openKnowledge}><span>Conhecimento</span><strong>Entender os movimentos →</strong></button><button type="button" onClick={goTreatment}><span>Tratamentos relacionados</span><strong>Acupressão · dietoterapia · acupuntura →</strong></button></div>
